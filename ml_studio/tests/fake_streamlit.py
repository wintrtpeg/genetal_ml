...
- **`key` 를 보지 않는다.** 위젯 값은 label 로만 고른다(`_pick`). 진짜
  streamlit 은 `key` 가 붙으면 session_state 에 있는 값을 쓰고 index·value
  인자를 무시하며, 사용자가 고른 값을 다시 그리기 **전에** 거기 써 둔다.
  이 차이 때문에 사이드바 레일이 통째로 안 먹는 것을 40여 조합이 전부
  통과하면서 못 잡았다 (25차). 그 부류는 여기서 실행으로 잡을 수 없어서
  `test_runtime_guards.test_widget_key_written_by_code_must_use_on_change`
  가 **형태로** 막는다 — 코드가 대입하는 키를 위젯 key 로 쓰면서 on_change 가
  없으면 실패한다.
